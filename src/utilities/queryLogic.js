 ...
 * 
 * @param {JSON} queryParams An object that is a JSON representation of all query string parameters
 * @param {JSON} templateTypes An object that maps each argument from the query string to a data type
 * 
 * @returns {JSON} Returns a JSON with the same key-value pairs as queryParams, except with the right data types
 */
function recastQuery (queryParams, templateTypes) {
    
    // Getting an array of each type in the templateTypes object
    let typeList = Object.keys(templateTypes);

    // recastQuery will contain the new list of queryParams with the
    // correct data types
    let recastQuery = {}
...
module.exports = {
    recastAthleteQuery : queryParams => recastQuery(queryParams, constants.ATHLETE_D_TYPES)
}